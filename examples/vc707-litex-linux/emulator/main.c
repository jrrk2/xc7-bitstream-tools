...
static void litex_putchar_raw(char c){
    uint32_t full;
    full = 1;
    while ((full & 0x1) != 0)
...
/* The console this drives is a terminal, and Linux emits bare '\n'.  The
   original dropped '\r' outright and added none, so every line after the
   first started where the last one ended.  Dropping incoming '\r' is kept --
   otherwise a source that already sends CRLF would get two -- and one is
   synthesised ahead of each '\n'. */
